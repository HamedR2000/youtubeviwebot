"""One-off: today's stories were already generated with Pexels stock photos
before the backgrounds swap landed. Re-render just the 3 stories with the
new AI-art background bank (same lines, same order) and host them + a
corrected manifest.json under a fresh tag, so the pending review can be
re-approved against updated URLs without re-touching the already-fine reel.
